...
wss.on('connection',(ws)=>{
  ws.on('message',raw=>{ let m; try{m=JSON.parse(raw)}catch{return}
    if(m.type==='create'){ const room={code:code(),host:ws,players:new Map(),phase:'lobby',day:0,night:0}; rooms.set(room.code,room); ws.room=room.code; send(ws,{type:'created',code:room.code}); push(room); return; }
